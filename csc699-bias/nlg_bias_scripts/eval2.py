...
def main():
	...
	parser.add_argument('--path',
						required=False,
						default='../../csc699/models/bert',
						help='BERT model location.')
	...
	if not os.path.exists(data_file + '_preds'):
		run_classifier = 'bash scripts/run_ensemble2.sh ' + params.model_type + ' ' + params.path + ' ' + no_ext_sample_name
		p = subprocess.Popen(run_classifier, shell=True)
		p.communicate()

	# Calculate ratios of pos/neu/neg samples for evaluation.
	if params.bias_dim != 'all':
		print('=' * 80)
		print('RESPECT')
		...
		print('RESPECT + OCCUPATION')
		p = subprocess.Popen('python scripts/analyze_generated_outputs.py --full_tsv_file ' + params.sample_file + 
						' --bias_dim "all" --model_type ' + params.model_type, shell=True)
		p.communicate()
